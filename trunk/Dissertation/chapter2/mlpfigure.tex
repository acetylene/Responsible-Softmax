\begin{figure}[ht]
	...
	\end{tikzpicture}
		\caption[Network graph of a multilayer perceptron.]{A graph model of a single hidden layer MLP with 5 inputs, 3 output and 6 hidden layer nodes. The output of each layer is determined by composing a linear map with a nonlinear map. The nonlinear map is determined at the outset. The linear map is determined through a set of learned weights.}
	\label{fig:MLP}
\end{figure}