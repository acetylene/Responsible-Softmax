...
%		\draw[thick,red] plot[samples=100, smooth, domain=-1:5,id=exp1] (\x,{5*exp(-(\x-1)^2/1)/(2*1*sqrt(2*3.14159))});                 % mean 1 sd 1
...
%\begin{align*}
%\frac{P\left(x-\Delta x<x_n<x,k_n=k\right)}{P\left(x-\Delta x < x_n <x+\Delta x\right)}&\approx\\ \frac{\pi_kf_k(x)\Delta x}{\sum_{k'} \pi_{k'}f_{k'}(x) \Delta x}&=P(x_n|k_n=k)
%\end{align*}
