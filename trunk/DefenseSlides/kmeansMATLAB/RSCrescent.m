...
%Positive values of SHIFT move clusters further, negative closer.
shift = -7.3;
...
for i = 1:4
    ...
    hold on
    scatter(testData(:,1),testData(:,2),10,testLabels,'o',...
        'MarkerEdgeColor','k','MarkerFaceColor','flat')
    %face color should be 'auto' or 'flat'
    hold off
...
for ii=[1 2 5]
    ...
    axis equal
    h(ii).Children.XLim = [xmin xmax];
    h(ii).Children.YLim = [ymin ymax];
end
a = colormap('lines');
c4 = a([1 3 5 7],:);
for ii=[1 2 3 5]
...
for ax =1:length(axs)
    axis(axs(ax),'equal');
    axs(ax).XLim = [xmin xmax];
    axs(ax).YLim = [ymin ymax];
    axs(ax).TickLabelInterpreter = 'latex';
    axs(ax).Title.Interpreter = 'latex';
end
